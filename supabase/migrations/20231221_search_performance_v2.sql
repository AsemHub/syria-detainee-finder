...
-- Improved Arabic text normalization function with safer character replacement
...
BEGIN
    ...
    -- Step 1: Convert special characters one by one
    ...
    -- Step 2: Remove kashida, tashkeel, and dagger alif
...
SELECT 
    ...
    -- Only store summary for large text fields
    ...
    -- Pre-computed document vector with carefully chosen weights
...
DECLARE
    ...
    search_term text;
    min_query_length constant int := 2;  -- Reduced minimum length for testing
    min_trigram_length constant int := 3;  -- Reduced minimum for trigram search
    debug_info jsonb;
    mv_count integer;
    sample_record jsonb;
...
BEGIN
    -- Get count of records in materialized view
    SELECT COUNT(*) INTO mv_count FROM detainees_search_mv;
    
    -- Get a sample record to verify structure
    SELECT row_to_json(d)::jsonb INTO sample_record 
    FROM detainees_search_mv d 
    LIMIT 1;
    
    -- Extract and validate parameters with explicit type casting and debugging
    ...
    -- Initialize debug info with raw input
    debug_info := jsonb_build_object(
        'rawInput', params,
        'extractedParams', jsonb_build_object(
            'search_query', p_search_query,
            'page_size', p_page_size,
            'cursor_id', p_cursor_id,
            'cursor_rank', p_cursor_rank,
            'cursor_date', p_cursor_date,
            'estimate_total', p_estimate_total,
            'include_secondary', p_include_secondary
        ),
        'mvCount', mv_count,
        'sampleRecord', sample_record,
        'minQueryLength', min_query_length,
        'minTrigramLength', min_trigram_length,
        'queryLength', length(trim(p_search_query))
    );
    ...
    -- Update debug info with query processing
    ...
    -- Get sample matches to help debug (limited to 5 records)
    WITH sample_matches AS (
        SELECT 
            d.id,
            d.full_name,
            d.primary_search_text,
            d.secondary_search_text,
            search_tsquery @@ d.document_vector as fts_match,
            d.primary_search_text LIKE '%' || search_term || '%' as primary_match,
            d.secondary_search_text LIKE '%' || search_term || '%' as secondary_match
        FROM detainees_search_mv d
        WHERE
    ...
    SELECT debug_info || jsonb_build_object('sampleMatches', jsonb_agg(row_to_json(s)::jsonb))
    INTO debug_info
    FROM sample_matches s;
    ...
    IF p_estimate_total AND p_cursor_id IS NULL THEN
        ...
        debug_info := debug_info || jsonb_build_object('totalCount', total_count);
    END IF;

    -- Improved ranking with better weights
...
RETURNS trigger AS $$
BEGIN
    -- Only refresh if it's been more than 15 minutes since last refresh
    IF NOT EXISTS (
        SELECT 1 
